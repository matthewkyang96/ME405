...
def USSENSOR():
    '''The Ultrasonic Sensor stops the robot if the robot comes within 10 cm of
    an object.  Pins A8 and A9 were used for the trigger and echo pins. A 
    shared variable is used to communicate with the back motor task to stop
    the motor if within 10 cm of an object.'''
...
##CONTROLLER TASK##
def controller():
    ''' The controller task executes the PID controller.  The gains are set up
    at initialization.  The controller returns a duty cycle level that the
    encoder motor will run at to balance the robot.'''
...
##REMOTE##
def remote():
    '''The remote tasks sets up an input pin to recieve data from the remote.
    The data is interpreted to turn the back motor on and off through a 
    shared variable.'''
...
def backmotor():
    '''The back motor task runs the driving motor of the robot.  PA0 and PA1 
    are set up to run the motor.  The back motor has two states either motor
    on or motor off.  The task initializes in the motor off state.  Shared
    variables between the tasks control which state the motor is in.'''
